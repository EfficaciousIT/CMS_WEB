...
public partial class frmTCdeatail : DBUtility
{
    ...
    {
        strQry = "exec usp_TCcertificate @command='SelectAll',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "'";
        dsObj = sGetDataset(strQry);

        int Leavegrid = sBindGrid(grdTrans, strQry);
        lblcount.Text = "Count :" + Convert.ToString(Leavegrid);
    ...
    public void FillDetailTransGrid()
    {
        try
        {
            ...
            //dsObj = sGetDataset(strQry);
                ...
                strQry = "exec usp_TCcertificate @command='Selectstudent',@intstudent_id='" + Convert.ToString(ddlStudent.SelectedValue) + "',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "'";
                dsObj = sGetDataset(strQry);
                grdTrans.DataSource = dsObj;
                grdTrans.DataBind();
    ...
    protected void grdTrans_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            ViewState["srno"] = this.grdTrans.DataKeys[e.RowIndex].Values[0].ToString();
            string srno = Convert.ToString(ViewState["srno"]);
           ...
           // strQry = "exec [usp_SchoolFeeCollectionNew_SP] @type='delete',@intID='" + Convert.ToString(ViewState["intID"]) + "',@intSchool_id='" + Convert.ToString(Session["School_id"]) + "'";
            strQry = "exec usp_TCcertificate @command='Deletestudent',@intstudent_id='" + Convert.ToString(ViewState["StudentID"]) + "',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "',@SRNo='" + srno + "'";
    ...
    protected void grdTrans_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            Session["StudentID"] = this.grdTrans.DataKeys[e.NewEditIndex].Values[1].ToString();
            
            DataSet dsObj = new DataSet();
            strQry = "";
            strQry = "exec usp_TCcertificate @command='Selectstudent',@intstudent_id='" + Convert.ToString(Session["StudentID"]) + "',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "'";
            dsObj = sGetDataset(strQry);
    ...
    protected void lnkEdit_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           // string userid = grdTrans.DataKeys[gvrow.RowIndex].Value.ToString();
            ...
            string STudentID = Convert.ToString(grdTrans.DataKeys[gvrow.RowIndex].Values[1]);
            Session["StudentID"] = STudentID;

            DataSet dsObj = new DataSet();
            strQry = "";
            strQry = "exec usp_TCcertificate @command='Selectstudent',@intstudent_id='" + STudentID + "',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "'";
            dsObj = sGetDataset(strQry);
            Session["rptStudentdetail"] = dsObj;
            Session["StudentID"] = STudentID;
            if (dsObj.Tables[0].Rows.Count > 0)
            {
                Response.Redirect("frmTCEdit.aspx", true);
            }
        }
        catch (Exception)
        {
