...
public partial class frmEVS : DBUtility
{
    ...
    protected void filldata()
    {
        try
        {
            string query1 = "Execute dbo.usp_Area @command='SelectEVSStan',@intSchool_id='" + Session["School_id"] + "' ";
            bool st = sBindDropDownList(Standard_id, query1, "vchStandard_name", "intstandard_id");

            string query2 = "Execute dbo.usp_Area @command='SelectEVSStan',@intSchool_id='" + Session["School_id"] + "' ";
            bool st1 = sBindDropDownList(Stdrop1, query2, "vchStandard_name", "intstandard_id");

            string Disquery = "Execute dbo.usp_Area @command='GridDiscipline',@intSchool_id='" + Session["School_id"] + "',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "'";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Button1.Text == "Submit")
            {
                ...
                strQry = "exec [usp_Area] @command='checkDisciplineExists',@VchName='" + Name + "',@intStandard_id='" + intStandard_id + "',@intSchool_id='" + Convert.ToString(Session["School_id"]) + "',@intAcademic_id='" + Convert.ToString(Session["AcademicID"]) + "'";
                ...
                {
                    itrque1 = "Execute dbo.usp_Area @command='InsertDiscipline',@VchName='" + Name + "',@intStandard_id='" + intStandard_id + "',@fltinsertIP='" + instip + "',@intSchool_id='" + Session["School_id"] + "',@intInsert_By='" + Session["user_id"] + "',@intAcademic_id='" + Session["AcademicID"] + "'";
            ...
            else if (Button1.Text == "Update")
            {
                try
                {
                    ...
                    {
                        itrque1 = "Execute dbo.usp_Area @command='UpdateDiscipline',@VchName='" + Name + "',@intStandard_id='" + intStandard_id + "',@IntUpdate_IP='" + Updateip + "',@intSchool_id='" + Session["School_id"] + "',@intUpdate_id='" + Session["user_id"] + "',@id='" + id + "',@intAcademic_id='" + Session["AcademicID"] + "'";

                        int result1 = sExecuteQuery(itrque1);

                        if (result1 != -1)
                        {
                            string display = "EVS subject Name Update Successfully!";
                            MessageBox(display);
                            Clear();
                            filldata();
    ...
    protected void SubReport_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            string strQry11 = "Execute dbo.usp_Area @command='DeleteDiscipline',@id='" + id + "',@intschool_id='" + Session["School_id"] + "',@fltDeleteIP='" + DeleteIP + "',@intDeleted_by='" + Session["user_id"] + "'";
    ...
    protected void SubReport_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            string strQry = "exec dbo.usp_Area @command='EditDiscipline',@id='" + id + "',@intschool_id='" + Session["School_id"] + "'";
            dsObj = sGetDataset(strQry);
            if (dsObj.Tables[0].Rows.Count > 0)
            {
                Button1.Text = "Update";
    ...
    protected void Stdrop1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (query2 == "A")
        {
            string Disquery = "Execute dbo.usp_Area @command='GridDiscipline',@intSchool_id='" + Session["School_id"] + "'";
            int grvDetail1 = sBindGrid(SubReport, Disquery);
        ...
        {
            string Disquery2 = "Execute dbo.usp_Area @command='GridDisciplineStan',@intSchool_id='" + Session["School_id"] + "',@intStandard_id='" + query2 + "'";
